class XbusPacket:
    # __init__ 에 인스턴스 생성될 때, live_data_list 만들기
    ...
    def feed_byte(self, byte):
        ...
        if self.got_first_byte and self.got_second_byte and len(self.buffer) >= 3:
            ...
            if self.is_packet_complete():
                ...
                # ----> self.live_data_list 추가하기
                self.live_data_list =self.on_data_available(self.buffer)
                # print('self.live_data_list : ', self.live_data_list)
    ...
    # 새로 추가 -- live_data_list 받아오기
    def return_live_data_list(self):
        live_data_list = self.live_data_list
        self.live_data_list = []
